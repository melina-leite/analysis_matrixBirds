...
load(here("models_outputs","models_traits.Rdata"))
```
...
```{r, eval=F}
hwhigh.spe <- glmer(cbind(occor, n.visit-occor) ~  
                        forest_site400*handwing + forest_land*handwing +
         (forest_site400 + forest_land|sp) + (1|landscape:sp) + (1|site:sp) + 
                        (handwing|landscape) + (handwing|site),
                      family=binomial, data=high.spe,
                      nAGQ = 1, control = glmerControl(optimizer = "bobyqa",
                                      optCtrl = list(maxfun = 500000)))

hwlow.spe <- glmer(cbind(occor, n.visit-occor) ~  
                      forest_site400*handwing + forest_land*handwing +
           (forest_site400 + forest_land|sp) + (1|landscape:sp) + (1|site:sp) + 
                      (handwing|landscape) + (handwing|site),
                    family=binomial, data=low.spe,
                    nAGQ = 1, control = glmerControl(optimizer = "bobyqa",
                                           optCtrl = list(maxfun = 500000)))
hwhigh.gen <- glmer(cbind(occor, n.visit-occor) ~  
                      forest_site400*handwing + forest_land*handwing +
                      (forest_site400 + forest_land|sp) + (1|landscape:sp) + (1|site:sp) + 
                      (handwing|landscape) + (handwing|site),
                    family=binomial, data=high.gen,
...
hwlow.gen <- glmer(cbind(occor, n.visit-occor) ~  
                     forest_site400*handwing + forest_land*handwing +
                     (forest_site400 + forest_land|sp) + (1|landscape:sp) + (1|site:sp) + 
                     (handwing|landscape) + (handwing|site),
                   family=binomial, data=low.gen,
...
# hand
hand <- c(5,15,35)
hand.high.spe <- (hand- mean(high.spe$handwingOrig, na.rm=T))/
                                sd(high.spe$handwingOrig, na.rm=T)
hand.low.spe <- (hand- mean(low.spe$handwingOrig, na.rm=T))/
                                sd(low.spe$handwingOrig, na.rm=T)
hand.high.gen <- (hand- mean(high.gen$handwingOrig, na.rm=T))/
                                sd(high.spe$handwingOrig, na.rm=T)
hand.low.gen <- (hand- mean(low.gen$handwingOrig, na.rm=T))/
                                sd(low.gen$handwingOrig, na.rm=T)
```

```{r}
# high.spe
phandhigh.spe <- expand.grid(forest_site400 = f400Orig.high,
                             forest_land = lands, 
                             handwing = hand)
newdata <- expand.grid(forest_site400 = f400st.high,
                       forest_land = land.high,
                       handwing = hand.high.spe)
phandhigh.spe$pred <- predict(hwhigh.spe, newdata = newdata, re.form=NA, 
                              type = "response")

# low.spe
phandlow.spe <- expand.grid(forest_site400 = f400Orig.low,
                             forest_land =lands, 
                             handwing = hand)
newdata <- expand.grid(forest_site400 = f400st.low,
                       forest_land = land.low,
                       handwing = hand.low.spe)
phandlow.spe$pred <- predict(hwlow.spe, newdata = newdata, re.form=NA, 
                             type = "response")


# high.gen
phandhigh.gen <- expand.grid(forest_site400 = f400Orig.high,
                             forest_land = lands, 
                             handwing = hand)
newdata <- expand.grid(forest_site400 = f400st.high,
                       forest_land = land.high,
                       handwing = hand.high.gen)
phandhigh.gen$pred <- predict(hwhigh.gen, newdata = newdata, re.form=NA, 
                              type = "response")

# low.gen
phandlow.gen <- expand.grid(forest_site400 = f400Orig.low,
                            forest_land =lands, 
                            handwing = hand)
newdata <- expand.grid(forest_site400 = f400st.low,
                       forest_land = land.low,
                       handwing = hand.low.gen)
phandlow.gen$pred <- predict(hwlow.gen, newdata = newdata, re.form=NA, 
                             type = "response")
```

```{r}
phand.spe <- bind_rows(list(high.spe = phandhigh.spe, 
                        low.spe = phandlow.spe), .id="group") %>%
  separate(group, c("matrix", "habitat")) 
phand.gen <- bind_rows(list(high.gen = phandhigh.gen, 
                            low.gen = phandlow.gen), .id="group") %>%
  separate(group, c("matrix", "habitat")) 
phand <- bind_rows(phand.spe, phand.gen) %>%
  mutate(habitat = fct_relevel(habitat, "spe", "gen"))
```

```{r}
labihand <- c(`5`= "5", `15`= "15", `35`= "35", `spe`= "Specialists",
              `gen` = "Generalists")
```

```{r}
fighand <- ggplot(phand, aes(x=forest_site400, y=pred, col=matrix,
                                        linetype=as.factor(forest_land))) + 
  geom_line(size=1.5) + 
  facet_grid(habitat~as.factor(handwing), labeller=as_labeller(labihand)) +
   scale_linetype_manual(name = "Landscape \n cover (%)", 
                 values = c("dotted",  "dashed","solid")) +
  scale_color_manual(name="Matrix quality", 
                     labels=c("High", "Low"),
  ...
  scale_x_continuous(name="Local forest cover (%)", breaks=c(30,60)) +
  scale_y_continuous(name="Occurrence probability", 
                     breaks = c(0,0.1,0.2,0.3), limits = c(0,0.35)) +
  theme(strip.text.x = element_text(size=12, margin = margin(r=2,l=2,t=2,b=2)),
        text = element_text(size=14),
        axis.text = element_text(size=12),
        legend.position = "right") +
  ggtitle("Handwing index")
fighand
```

```{r}
fighand2 <- phand %>% filter(forest_land == 30) %>%
...
```{r, eval=F}
jpeg("figures/trait_handwing.jpg", width = 16, height = 10, res=150, units="cm" )
figinse
dev.off()
```
...
```{r}
AICtab(bodyhigh.spe, diethigh.spe, nesthigh.spe, low_strathigh.spe, hwhigh.spe,
...
r2high.spe <- bind_rows(list(
              ...
              inse_size = r2.calc(insehigh.spe, atrib= "inse"),
              handwing = r2.calc(hwhigh.spe, atrib="handwing")), .id="trait" )
kable(r2high.spe)
```
...
```{r}
AICtab(bodylow.spe, dietlow.spe, nestlow.spe, low_stratlow.spe, hwlow.spe,
...
r2low.spe <- bind_rows(list(
              ...
              inse_size = r2.calc(inselow.spe, atrib= "inse"),
              handwing = r2.calc(hwlow.spe, atrib="handwing")), .id="trait" )
kable(r2low.spe)
```
...
```{r}
AICtab(bodyhigh.gen, diethigh.gen, nesthigh.gen, low_strathigh.gen, hwhigh.gen, 
...
r2high.gen <- bind_rows(list(
              ...
              inse_size = r2.calc(insehigh.gen, atrib= "inse"),
              handwing = r2.calc(hwhigh.gen, atrib="handwing")), .id="trait" )
kable(r2high.gen)
```
...
```{r}
AICtab(bodylow.gen, dietlow.gen, nestlow.gen, low_stratlow.gen, hwlow.gen,
...
r2low.gen <- bind_rows(list(
              ...
              inse_size = r2.calc(inselow.gen, atrib= "inse"),
              handwing = r2.calc(hwlow.gen, atrib="handwing")), .id="trait" )
kable(r2low.gen)
```
...
```{r}
jpeg(here("figures","traits_diet_nest_body.jpg"), width = 20, height = 18, res=150, units="cm" )
...
tiff(here("figures","traits_diet_nest_body.tiff"), width = 18, height = 16, res=300, units="cm" )
(figdiet2+legenda +plot_layout(widths = c(1,0.07)))/(figbody+fignest) 
dev.off()

ggsave(here("figures","traits_diet_nest_body.pdf"), device="pdf",
...
hwhigh.spe2 <- glmer(cbind(occor, n.visit-occor) ~  
                      forest_site400 + forest_land +handwing +
                      (forest_site400 + forest_land|sp) + (1|landscape:sp) + (1|site:sp) + 
                      (handwing|landscape) + (handwing|site),
                    family=binomial, data=high.spe,
                    nAGQ = 1, control = glmerControl(optimizer = "bobyqa",
...
hwlow.spe2 <- glmer(cbind(occor, n.visit-occor) ~  
                     forest_site400 + forest_land + handwing +
             (forest_site400 + forest_land|sp) + (1|landscape:sp) + (1|site:sp) + 
                     (handwing|landscape) + (handwing|site),
                   family=binomial, data=low.spe,
...
hwhigh.gen2 <- glmer(cbind(occor, n.visit-occor) ~  
                       forest_site400 + forest_land +handwing +
            (forest_site400 + forest_land|sp) + (1|landscape:sp) + (1|site:sp) + 
                       (handwing|landscape) + (handwing|site),
                     family=binomial, data=high.gen,
...
hwlow.gen2 <- glmer(cbind(occor, n.visit-occor) ~  
                      forest_site400 + forest_land + handwing +
            (forest_site400 + forest_land|sp) + (1|landscape:sp) + (1|site:sp) + 
                      (handwing|landscape) + (handwing|site),
                    family=binomial, data=low.gen,
...
save(bodyhigh.spe, bodyhigh.gen, bodylow.spe, bodylow.gen,
     ...
     hwhigh.spe, hwhigh.gen, hwlow.spe, hwlow.gen,
     ...
     hwhigh.spe2, hwhigh.gen2, hwlow.spe2, hwlow.gen2,
     file= here("models_outputs","models_traits.Rdata"))
```
