...
Organization of the datasets for the environmental, species and site information.
...
```{r}
here("data", "species_site_occ.csv")
...
data %<>% tidyr::pivot_longer(cols = cols_to_choose, 
                              names_to = "sp", 
                              values_to = "occor")
str(data)
...
Landuse types information for each landscape. This information is used to describe matrix quality (heterogeneity and contrast types) and to compare matrices among landscapes considered of low and high-quality for bird assemblages.
```{r}
landuse <- read.csv(here("data", "landuse_landscapes_2km.csv"))
str(landuse)
```

Environmental site information for forest cover (local at different buffer radius and landscape scale), and coordinates for the sites and the centroids of the landscapes.
```{r}
env <- read.csv(here("data", "sites_information.csv"))
str(env)
```
...
```{r}
# use group_by in dataset to scale all the continuous variables - see if it is possible - it will save space OR make a function!
...
```{r}
save(landuse, env, high.spe, high.gen, low.spe, low.gen, 
